...
export default function Orders() {
  ...
  const saveOrder = async () => {
    ...
    if (itemsErr) {
      ...
      toast.error('Falha ao salvar itens. Pedido foi revertido.');
      setSaving(false);
      return;
    }
  ...
  const updateStatus = async (orderId: string, newStatus: string, currentStatus: string) => {
    if (!isStatusChangeAllowed(currentStatus, newStatus)) {
      if (newStatus === currentStatus) return;
      toast.error('Pedido entregue nao pode retornar para status anterior.');
      return;
    }
    ...
    // Apply stock movement only once, when transitioning into delivered state.
    if (shouldApplyStockMovement(currentStatus, newStatus)) {
      ...
      if (orderItemsData) {
        const stockMovements = buildDeliveredStockMovements(
          orderItemsData.map(item => ({
            material_id: item.material_id,
            color: item.color,
            weight_g: item.weight_g,
            qty: item.qty,
            productCategory: String((item.products as any)?.category ?? ''),
          })),
        );
        ...
        for (const movement of stockMovements) {
          const { data: stockRow } = await supabase
            ...
            .eq('color', movement.color)
            .maybeSingle();

          if (stockRow) {
            await supabase.from('stock').update({ qty_g: (stockRow.qty_g ?? 0) + movement.qty_delta_g }).eq('id', stockRow.id);
          } else {
            await supabase.from('stock').insert({
              workspace_id: workspaceId!,
              material_id: movement.material_id,
              color: movement.color,
              qty_g: movement.qty_delta_g,
              min_g: 0,
            });
          }
          ...
          await supabase.from('stock_movements').insert({
            ...
            material_id: movement.material_id,
            color: movement.color,
            qty_delta_g: movement.qty_delta_g,
