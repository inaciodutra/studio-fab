...
export default function Config() {
  ...
  const sendInvite = async () => {
    ...
    setInviting(true);
    const emailToInvite = normalizeInviteEmail(inviteEmail);
  ...
  const changeRole = async (userId: string, newRole: AppRole) => {
    ...
    if (!target) return;
    const adminCount = teamMembers.filter(m => m.roles.includes('ADMIN')).length;
    if (isLastAdminDemotionOrRemoval(adminCount, target.roles, newRole)) {
      toast.error('Nao e permitido rebaixar o ultimo admin do workspace.');
      return;
    }

    const { data, error } = await supabase.functions.invoke('manage-team-member', {
      body: { action: 'change_role', member_id: userId, new_role: newRole },
    });
  ...
  const removeMember = async (userId: string, memberName: string) => {
    ...
    if (isSameUser(profile?.id, userId)) {
      toast.error('Voce nao pode remover a propria conta.');
      return;
    }
    const target = teamMembers.find(m => m.id === userId);
    if (!target) return;
    const adminCount = teamMembers.filter(m => m.roles.includes('ADMIN')).length;
    if (isLastAdminDemotionOrRemoval(adminCount, target.roles)) {
      toast.error('Nao e permitido remover o ultimo admin do workspace.');
      return;
    }

    const { data, error } = await supabase.functions.invoke('manage-team-member', {
      body: { action: 'remove_member', member_id: userId },
    });
    if (error) { toast.error(error.message); return; }
    if (data?.error) { toast.error(data.error); return; }
